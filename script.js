...
document.addEventListener("DOMContentLoaded", () => {
    ...
    window.addEventListener('wheel', (event) => {
        if (event.deltaY > 0 && currentIndex < timelineData.length - 1) {
            currentIndex++;
        } else if (event.deltaY < 0 && currentIndex > 0) {
            currentIndex--;
        }
        offsetX = currentIndex * -450; // Horizontal movement
        timeline.style.transform = `translateX(${offsetX}px) translateZ(${currentIndex * -200}px)`;
    });
});
